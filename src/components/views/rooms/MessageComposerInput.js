...
export default class MessageComposerInput extends React.Component {
    ...
    componentWillUnmount() {
        const input = document.querySelector('.mx_MessageComposer_input');
        window.removeResizeListener(input, window.inputResize);
    ...
    componentDidMount() {
        (function() {
            ...
            window.inputResize = function() {
                const input = document.querySelector('.mx_MessageComposer_input');
                const scrollHeight = document.querySelector('.mx_AutoHideScrollbar.mx_ScrollPanel.mx_RoomView_messagePanel').scrollHeight;
                if (input.offsetHeight > 90) {
                    document.querySelector('.mx_AutoHideScrollbar.mx_ScrollPanel.mx_RoomView_messagePanel').scrollTop = scrollHeight;
                }
            };

            const input = document.querySelector('.mx_MessageComposer_input');
            window.addResizeListener(input, () => {
                console.log('input resize');
                window.inputResize();
            });
    }
