...
module.exports = React.createClass({
    ...
    componentWillMount: function() {
        ...
        const enablePresenceByHsUrl = SdkConfig.get()[
        ...
        if (
            ...
            enablePresenceByHsUrl[hsUrl] !== undefined
        ) {
    ...
    onRoomStateEvent: function(event, state) {
        if (
            event.getRoomId() === this.props.roomId &&
    ...
    _updateListNow: function() {
        ...
        newState.filteredJoinedMembers = this._filterMembers(
            newState.members,
            'join',
            this.state.searchQuery
        );
        newState.filteredInvitedMembers = this._filterMembers(
            newState.members,
            'invite',
            this.state.searchQuery
        );
    ...
    roomMembers: function() {
        ...
        const filteredAndSortedMembers = allMembers.filter(m => {
            return (
                ...
                (!ConferenceHandler ||
                    (ConferenceHandler &&
                        !ConferenceHandler.isConferenceUser(m.userId)))
            );
        });
    ...
    _createOverflowTileJoined: function(overflowCount, totalCount) {
        return this._createOverflowTile(
            overflowCount,
            totalCount,
            this._showMoreJoinedMemberList
        );
    },
    ...
    _createOverflowTileInvited: function(overflowCount, totalCount) {
        return this._createOverflowTile(
            overflowCount,
            totalCount,
            this._showMoreInvitedMemberList
        );
    },
    ...
    _showMoreInvitedMemberList: function() {
        this.setState({
            truncateAtInvited:
                this.state.truncateAtInvited + SHOW_MORE_INCREMENT
        });
    },
    ...
    memberSort: function(memberA, memberB) {
        ...
        const nameA =
            memberA.name[0] === '@' ? memberA.name.substr(1) : memberA.name;
        const nameB =
            memberB.name[0] === '@' ? memberB.name.substr(1) : memberB.name;
    ...
    onSearchQueryChanged: function(searchQuery) {
        this.setState({
            searchQuery,
            filteredJoinedMembers: this._filterMembers(
                this.state.members,
                'join',
                searchQuery
            ),
            filteredInvitedMembers: this._filterMembers(
                this.state.members,
                'invite',
                searchQuery
            )
        ...
        if (room) {
            return room.currentState
                ...
                .filter(function(e) {
                    ...
                    // already added them.
                    const memberEvent = room.currentState.getInviteForThreePidToken(
                        e.getStateKey()
                    );
    ...
    _getChildrenJoined: function(start, end) {
        return this._makeMemberTiles(
            this.state.filteredJoinedMembers.slice(start, end)
        );
    },
    ...
    _getChildCountInvited: function() {
        return (
            this.state.filteredInvitedMembers.length +
            (this._getPending3PidInvites() || []).length
        );
    },

    isInRoom(room) {
        const dmRooms = DMRoomMap.shared().getUserIdForRoomId(
            this.props.roomId
        );
        if (dmRooms || !room || room.getMyMembership() !== 'join') return false;
        return true;
    },
    ...
    render: function() {
        ...
        return (
            <div className='mx_MemberList'>
                {this.isInRoom(room) && inviteButton}
                <AutoHideScrollbar>
                    <div className='mx_MemberList_wrapper'>
                        <TruncatedList
                            ...
                            createOverflowElement={
                                this._createOverflowTileJoined
                            }
